...
class Application {
    ...
    public function run() : string {
        ...
        if(isset($routeArray[1]) && $routeArray[1] != '') {
            if (!preg_match('/^[a-zA-Z0-9]+$/', $routeArray[1])) {
                return "Недопустимые символы в имени контроллера"; //Контроллер: спец символы
            }
        ...
        if(class_exists($this->controllerName)){
            if(isset($routeArray[2]) && $routeArray[2] != '') {
                if (!preg_match('/^[a-zA-Z0-9]+$/', $routeArray[2])) {
                    return "Недопустимые символы в имени метода"; //Метод: спец символы
                }
